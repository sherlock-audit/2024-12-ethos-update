...
const contractsConfigMap: ContractConfig = {
  ...
  reputationMarket: {
    ...
    libraries: ['contracts/utils/LMSR.sol:LMSR'],
  },
  review: {
...
async function main(): Promise<void> {
  ...
  let contractObj: Awaited<ReturnType<typeof ethers.getContractFactory>>;

  if (contractConfig.libraries?.length) {
    const libraryAddresses: Record<string, string> = {};

    for (const libraryKey of contractConfig.libraries) {
      console.log('📚 Deploying library...', libraryKey);
      ...
      console.log(`📚 Library ${libraryKey} deployed to ${libraryAddress}`);
    }

    console.log('🎓 Deploying contract with libraries...');
