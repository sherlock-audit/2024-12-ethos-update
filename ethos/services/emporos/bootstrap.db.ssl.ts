...
import { config, dbConfig } from './app/config/config.server.ts';
...
function verifyPrismaUrl(caPath: string, p12Path: string): void {
  ...
  if (config.EMPOROS_DB_URL.includes(dbParams)) {
    const configParams = new URL(config.EMPOROS_DB_URL).search;
    console.error(
      `Bootstrap DB SSL: EMPOROS_DB_URL (${configParams}) does not match the SSL configuration (${dbParams})`,
    );
  }
}
